...
#define ugc getchar
    ...
    __1(>, string, _nt(s)) __1(>, ld, _nd(s)) __1(>, ll, _ni(s))
        __1(>, int, _ni(s)) __1(>=, string, _nln(s))
            __1(<, const int, write_int(s)) __1(<, const ll, write_int(s))
...
bool bfs(int removed) {
    ...
    while (!q.empty()) {
        ...
        for (auto nxt : adj[cur]) {
            if (nxt.second == removed) continue;
            if (!vis[nxt.first]) q.push_back(nxt.first);
        }
    }
    return false;
}
