...
bool comp(vector<int> &a, vector<int> &b) { return a[ind] < b[ind]; }
int check(vector<int> a, vector<int> b) {
    int ret = 0;
    for (int i = 2; i <= n; i++) {
        if (a[i] + k == b[i] && a[i - 1] + k == b[i - 1])
            ret++;
        else if (a[i] - k == b[i] && a[i - 1] - k == b[i - 1])
            ret++;
    }
    return ret;
}
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < m; j++) {
                for (int num = 1; num < counts[i][j].size(); num++) {
                    if (counts[i][j][num] - 1 == counts[i][j][num - 1]) ans++;
                }
            }
        }
    }
