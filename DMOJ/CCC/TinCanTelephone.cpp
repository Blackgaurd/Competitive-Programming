...
int gcd(int a, int b, int c) { return __gcd(a, __gcd(b, c)); }
struct line {
    // ax + by + c = 0
    int a, b, c;
    int px, py, qx, qy;
    ...
    bool bound(const line &line_, double interx, double intery) {
        return interx >= min(line_.px, line_.qx) &&
               interx <= max(line_.px, line_.qx) &&
               intery >= min(line_.py, line_.qy) &&
               intery <= max(line_.py, line_.qy);
    }

    bool intersect(const line &other) {
        double interx = (double)(b * other.c - other.b * c) /
                        (double)(a * other.b - other.a * b);
        double intery = (double)(other.a * c - a * other.c) /
                        (double)(a * other.b - other.a * b);
