...
int main() {
    ...
    while (!cams.empty()) {
        ...
        for (int x = cam.row; x <= n; x++) {
            if (arr[x][cam.col] == 'W')
                break;
            else if (is_conveyor(arr[x][cam.col]) == -1)
                arr[x][cam.col] = 'C';
        }
        ...
        for (int y = cam.col; y <= n; y++) {
            if (arr[cam.row][y] == 'W')
                break;
            else if (is_conveyor(arr[cam.row][y]) == -1)
                arr[cam.row][y] = 'C';
        }
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            if (!vis(arr[cur.row + dx[i]][cur.col + dy[i]])) {
                if (dis[cur.row + dx[i]][cur.col + dy[i]] >
                    dis[cur.row][cur.col] + 1) {
                    dis[cur.row + dx[i]][cur.col + dy[i]] =
                        dis[cur.row][cur.col] + 1;
