...
int main(){
    ...
    do{while((n=getchar())<48); for(n-=48; 48<=(_=getchar()); n=(n<<3)+(n<<1)+_-48);}while(0);
    for (int i=0; i<n; i++){
        // head website
                    ...
                    if (t.substr(start, 4) == "HREF"){
                        for (int end=6; end<t.length()-start; end++){
                            if (t[start + end] == '"'){
                                ...
                                adj[head].push_back(t.substr(start+6, end-6));
                                if (flead(head) != flead(t.substr(start+6, end-6))){
                                    p[t.substr(start+6, end-6)] = flead(head);
                                }
                                cout << "Link from " << head << " to " << t.substr(start+6, end-6) << '\n';
                                break;
                            }
                        }
                    }
    ...
    while (true){
        ...
        else cout << "Can't surf from " << a << " to " << b << ".\n";
    }
